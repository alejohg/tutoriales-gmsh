...
def grupos_fisicos(archivo, dim=-1):
    ...
    nodos = [gmsh.model.mesh.getNodesForPhysicalGroup(dims[i], tags[i])[0] for i in range(len(dims))]
...
def obtener_nodos(archivo, nombre_grupo):
    ''' Lee un archivo de extensión .msh y obtiene los nodos asociados al grupo físico de nombre "nombre_grupo".
    '''
