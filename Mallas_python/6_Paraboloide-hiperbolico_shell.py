...
p1 = geom.addPoint(x, -1/2, f(x, -1/2))
p2 = geom.addPoint(x, 1/2, f(x, 1/2))
cy, cz = obtener_pc_bezier(a, b, c, [-1/2, f(x, -1/2)], [1/2, f(x, 1/2)])
pc1 = geom.addPoint(x, cy, cz)  # Punto de control de la primer parábola

pp1 = geom.addPoint(x, 0, -1/4)  # Vértice de la parábola

b1 = geom.addBezier([p1, pc1, p2])  # Se crea la parábola como una curva de
...
p3 = geom.addPoint(0, y, f(0, y))

cx, cz = obtener_pc_bezier(a, b, c, [1/2, f(1/2, y)], [0, f(0, y)])
pc2 = geom.addPoint(cx, y, cz)
pp2 = geom.addPoint(1/4, y, f(1/4, y))  # Punto sobre la parábola para verificar
b2 = geom.addBezier([p2, pc2, p3])
...
p4 = geom.addPoint(x, -1/2, f(x, -1/2))
cy, cz = obtener_pc_bezier(a, b, c, [-1/2, f(x, 1/2)], [1/2, f(x, -1/2)])
pc3 = geom.addPoint(x, cy, cz)  # Punto de control de la primer parábola

pp3 = geom.addPoint(x, 0, 0)  # Vértice de la parábola

b3 = geom.addBezier([p3, pc3, p4])  # Se crea la parábola como una curva de
                                    # Bezier con pto inicial, final y de control
...
pp4 = geom.addPoint(1/4, y, f(1/4, y))  # Punto sobre la parábola para verificar
b4 = geom.addBezier([p4, pc4, p1])    
